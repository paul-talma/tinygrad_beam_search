...
def traced_beam_search(s, rawbufs, amt: int, allow_test_size=True, disable_cache=None):
  import tinygrad.codegen.opt.search as search_mod
  from tinygrad.device import Device
  from tinygrad.helpers import CACHELEVEL, IGNORE_BEAM_CACHE, diskcache_get, diskcache_put, getenv
  ...
  trace = {
    "kernel_id": s.ast.key.hex() if isinstance(s.ast.key, (bytes, bytearray)) else str(s.ast.key),
  ...
  if search_mod.beam_pool is None and (workers := getenv("PARALLEL", default_parallel)):
  ...
  try:
    ...
    while not exiting:
      candidates = flatten([search_mod.get_kernel_actions(si, include_0=False).values() for si, _ in beam])
      timed = []
      step_records = []
      least_compute_ops = math.inf

      pool_map = search_mod.beam_pool.imap_unordered if search_mod.beam_pool is not None else map
      ...
      trace["steps"].append({
        ...
        "selected_beam": [
          {"runtime_s": runtime_s, "applied_opts": normalize_opts(scheduler.applied_opts)}
...
def run_once(op_fn, beam: int, cachelevel: int) -> tuple[float, int, str, list[dict], list[dict]]:
  ...
  return GlobalCounters.time_sum_s * 1e6, GlobalCounters.kernel_count, sig, sources, pop_beam_traces()


def _run_with_optional_regeneration(op_name: str, op_fn, regenerate: bool, beam: int, cachelevel: int) -> tuple[float, int, str, list[dict], list[dict]]:
  if regenerate:
    op_fn = make_conv_op(op_name, cachelevel)
  return run_once(op_fn, beam, cachelevel)


def measure(op: str, beam: int, warmup: int, runs: int, cachelevel: int, regenerate: bool, include_sources: bool, include_beam_exec_time: bool) -> dict:
  ...
  for _ in range(runs):
    t_us, kernel_count, sig, sources, beam_traces = _run_with_optional_regeneration(op, op_fn, regenerate, beam, cachelevel)
...
def print_beam_exec_time_traces(result: dict, top_k: int) -> None:
  for run_idx, traces in enumerate(result.get("beam_traces_per_run", []), start=1):
    ...
    for kernel_idx, trace in enumerate(traces, start=1):
      ...
      print(f"  final: {trace['final_runtime_s'] * 1e6:.3f} us opts={trace['final_applied_opts']}")
      for step in trace["steps"]:
        ...
        if step["selected_beam"]:
          ...
          for rank, cand in enumerate(step["selected_beam"], start=1):
            print(f"        {rank:2d}. exec={cand['runtime_s'] * 1e6:9.3f} us opts={cand['applied_opts']}")
...
def main() -> None:
  ...
  parser.add_argument("--beam_exec_time", action="store_true", help="Print per-step beam-search candidate execution times.")
